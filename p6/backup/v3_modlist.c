...
static ssize_t mi_modulo_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
    ...
    *off += len; /* Update the file pointer */
    ...
    if (sscanf(commandBuffer, "add %d", &newNumber) == 1)
    {
        ...
        while (m_list->consumidores > 0)
        {
            m_list->productores_waiting++;
            up(&m_list->mtx);
            
            if (down_interruptible(&m_list->productores_sem)) //Activamos el semáforo de productores
            {
                down(&m_list->mtx);
                m_list->productores_waiting--;
                up(&m_list->mtx);
        ...
        printk(KERN_INFO "mi_modulo_write 3\n");

        insert_item(newNumber, &m_list->links);
        printk(KERN_INFO "mi_modulo_write 4\n");
...
static int mi_modulo_open(struct inode *inode, struct file *file)
{
    ...
    printk(KERN_INFO "En mi_modulo_open 1 \n");
    /**
...
{
    printk(KERN_INFO "En mi_modulo_release 1 \n");
