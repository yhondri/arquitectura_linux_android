...
//SMP-safe
struct semaphore my_locker; //Operaciones generales, no solo de la lista.
...
struct list_item
{
    ...
    int productores;
    int consumidores;
    struct semaphore mtx; //Para cambiar contadores, productores, consumidores etc... de la lista.
...
void insert_item(int new_number, struct list_item *m_list_head);
void remove_item(int item, struct list_item *m_list);
void clean_up(void);
int create_new_list(char *list_name);
int delete_list(char *list_name, struct list_item *m_list);
...
static ssize_t mi_modulo_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
    ...
    *off += len; /* Update the file pointer */
    ...
    {
        insert_item(newNumber, m_list);
        return len;
    }
    else if (sscanf(commandBuffer, "remove %d", &newNumber) == 1)
    {
        remove_item(newNumber, m_list);
        return len;
    }
...
static ssize_t mi_modulo_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
    ...
    //Si hay productores introduciendo números, esperamos.
    ...
    //Si hay productores esperando, liberamos el semáforo de producctores.
...
static int mi_modulo_open(struct inode *inode, struct file *file)
{
    ...
    if (file->f_mode & FMODE_WRITE) //Productor - Añadir elementos.
    {
        m_list->productores++;

        printk(KERN_INFO "En mi_modulo_open 2.1 \n");
    }
    else if (file->f_mode & FMODE_READ) //Consumidor - Mostrar elementos por pantalla.
    {
        m_list->consumidores++;
...
static int mi_modulo_release(struct inode *inode, struct file *file)
{
    ...
    if (file->f_mode & FMODE_WRITE) //Abierto por un productor.
    {
        m_list->productores--;
    }
    else if (file->f_mode & FMODE_READ) //Consumidor
    {
        m_list->consumidores--;
    }
...
int create_new_list(char *list_name)
{
    ...
    sema_init(&new_list_item->productores_sem, 1);
    sema_init(&new_list_item->consumidores_sem, 1);
    ...
    if (new_entry == NULL)
    {
        delete_list(list_name, new_list_item);
        printk(KERN_INFO "No se ha podido crear la entrada\n");
        return -ENOMEM;
    }

    list_add_tail(&new_list_item->links, &mylist); //Insertamos el item a la lista.
...
int delete_list(char *list_name, struct list_item *m_list)
{
    printk(KERN_INFO "En delete_list %s\n", list_name);
    ...
    printk(KERN_INFO "En delete_list 1n");
...
static ssize_t control_write(struct file *file, const char __user *buf, size_t len, loff_t *off)
{
    ...
    else if (sscanf(kbuffer, "delete %s", command_name))
    {
        struct list_item *m_list = (struct list_item *)PDE_DATA(file->f_inode);

        if (m_list)
        {
            return delete_list(command_name, m_list);
        }
        else
        {
...
void remove_item(int item, struct list_item *m_list)
{
    ...
    list_for_each_safe(current_head, auxiliar, &m_list->links)
    {
        current_item = list_entry(current_head, struct list_item, links);
