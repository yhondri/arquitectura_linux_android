						   ...
						   size_t len, loff_t *off)
{
	...
	char kbuf[BUFFER_MAX_LENGTH]; //Creamos el buffer donde vamos a copiar la cadena alojada en user_buffer.
	char *kbufAuxiliar = kbuf;	//Para parsear cada comando.
	unsigned char messages[NR_LEDS][NR_BYTES_BLINK_MSG];
	const char *command_pattern = "%u:0x%x"; //%u integer as unsigned : %x entero en base 16.
	char *commandToken;
	unsigned int num_led;
	unsigned int led_color;

	//1 Copiar cadena alojada en user_buffer a kbuf.
	if (copy_from_user(kbuf, user_buffer, len))
	{
		printk(KERN_ERR "There was a failure while copying data from/to userspace, probably caused by an invalid pointer reference.");
		goto out_error;
	}

	//1.1 Incluir el terminador '\0'
	kbuf[len] = "\0";

	/**
	 The strsep() function locates, in the string referenced by *stringp, the first occurrence of any character
	  in the string delim (or the terminating \0 character) and replaces it with a \0. 
	  The location of the next character after the delimiter character (or NULL, if the end of 
	  the string was reached) is stored in *stringp. The original value of  *stringp is returned.
	*/
	...
	if (len == 1)
	{
						messages[num_led][3] = ((led_color >> 16) & 0xff);
				messages[num_led][4] = ((led_color >> 8) & 0xff);
				messages[num_led][5] = (led_color & 0xff);

				goto salto_lectura;
	}

	printk(KERN_ERR "Comando introducito %zu\n", len);

	while ((commandToken = strsep(&kbufAuxiliar, ",")) != NULL)
	{
		if (sscanf(commandToken, command_pattern, &num_led, &led_color) == 2)
		{
			if(num_led < 0 || num_led > 7)
			{
				retval = -EINVAL;
				goto invalid_command_error;
			} else {
				//Copiamos de la implementación original la aplicación de máscara de bits.
				messages[num_led][3] = ((led_color >> 16) & 0xff);
				messages[num_led][4] = ((led_color >> 8) & 0xff);
				messages[num_led][5] = (led_color & 0xff);
			}
		...
		{
			retval = -EINVAL;
			//Se ha introducito un parámetro inválido.
			goto invalid_command_error;
		}
	}
	...
	for (i = 0; i < NR_LEDS; i++)
	{
		...
		message[0] = '\x05';
		message[1] = 0x00;
		message[2] = i; //Número del led a modificar.
...
out_error:
	kfree(message);
invalid_command_error: //Cuando no hemos inicializado ninguna variable ni hemos reservado con vmalloc.
